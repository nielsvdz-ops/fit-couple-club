...
function getGoalAdjustment(goal) {
  ...
  if (clean.includes("fat") || clean.includes("loss")) return goalAdjustments.fat_loss;
  if (clean.includes("muscle") || clean.includes("gain") || clean.includes("build")) return goalAdjustments.lean_muscle;
  if (clean.includes("performance")) return goalAdjustments.performance;
...
function translateGoalLabel(goal, language) {
  ...
  if (clean.includes("muscle") || clean.includes("build")) return "Spieropbouw";
...
export default function RecipesClient({ membershipType }) {
  ...
  const copy = {
    en: {
      ...
      heroText: "Enter your sex, age, weight, height, activity level, and goal.",
      sex: "Sex",
      male: "Male",
      female: "Female",
      age: "Age",
    ...
    nl: {
      ...
      heroText: "Vul je geslacht, leeftijd, gewicht, lengte, activiteit en doel in.",
      sex: "Geslacht",
      male: "Man",
      female: "Vrouw",
      age: "Leeftijd",
    ...
    <div style={wrap}>
      <section style={heroCard}>
            ...
            >
              {visiblePlans.map((plan, index) => (
                <option key={plan.id} style={optionStyle} value={index}>
                  {translatePlanTitle(plan.title, language)}
                </option>
              ))}
            </select>
          </Field>
        ...
        <div style={goalInfoGrid}>
          <InfoCard label={t.targetCalories} value={`${personalTargets.targetCalories} kcal`} />
          <InfoCard label={t.maintenance} value={`${personalTargets.maintenanceCalories} kcal`} />
      ...
      <section style={panel}>
        <div style={sectionHead}>
          <div>
            ...
            <p style={sectionText}>
              {translatePlanDescription(selectedPlan?.description, language)}{" "}
        ...
        {selectedDayPlan && (
          <div style={mealSection}>
            <div style={summaryGrid}>
              <SummaryCard label={t.dailyCalories} value={`${personalTargets.targetCalories} kcal`} />
            ...
            <div style={mealGrid}>
              {selectedDayPlan.meals.map((meal) => (
                <div key={meal.mealName} style={mealCard}>
                  ...
                  <div style={macroRow}>
                    <span>{t.protein}: {scaleMacro(meal.protein, scale)}</span>
                    <span>{t.carbs}: {scaleMacro(meal.carbs, scale)}</span>
                    <span>{t.fats}: {scaleMacro(meal.fats, scale)}</span>
                  </div>
                  ...
                  <div style={detailBlock}>
                    ...
                    <ul style={bulletList}>
                      {meal.ingredients.map((item) => (
                        <li key={item}>{translateMealText(item, language)}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div style={detailBlock}>
                    ...
                    <ol style={orderedList}>
                      {meal.steps.map((step) => (
                        <li key={step}>{translateMealText(step, language)}</li>
                      ))}
                    </ol>
                  </div>
                </div>
...
const heroCard = {
  background:
    "linear-gradient(135deg, rgba(176,0,0,0.16), rgba(255,255,255,0.03))",
...
const dayTabStyle = (active) => ({
  flex: "0 0 auto",
  padding: "12px 16px",
  borderRadius: "14px",
  border: active
    ? "1px solid rgba(176,0,0,0.8)"
    : "1px solid rgba(255,255,255,0.08)",
  background: active ? "#b00000" : "rgba(255,255,255,0.04)",
  color: "white",
  fontWeight: "900",
  cursor: "pointer",
  fontSize: "14px",
...
const summaryCard = {
  background:
    "linear-gradient(135deg, rgba(176,0,0,0.10), rgba(255,255,255,0.04))",
