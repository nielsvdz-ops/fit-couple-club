...
const GOAL_RULES = {
  ...
  "Beginner Body Reset": {
    ...
    nutritionHint: "Simple high-protein eating structure and routine building",
  },
};
...
function inferNutritionStyle(goal) {
  ...
  if (goal === "Tone & Shape Body") return "High protein + controlled maintenance";
  if (goal === "Beginner Body Reset") return "Simple high-protein structure";
  return "Balanced maintenance nutrition";
}

function inferCardioRecommendation(goal, lifestyle) {
  if (goal === "Lose Fat") return "Optional cardio 3–4x/week for 20–30 minutes";
  if (goal === "Maintain Athletic Lifestyle") return "Optional cardio 2–3x/week";
  if (lifestyle === "Highly Active") return "Keep cardio moderate to protect recovery";
...
export function getRecommendedDefaults({ goal, focus, bodyType, level }) {
  ...
  if (bodyType === "Plus Size" && goal === "Beginner Body Reset") recommendedDays = 3;
