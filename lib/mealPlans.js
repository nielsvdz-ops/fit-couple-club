...
function adjustMealForGoal(meal, goal) {
  ...
  return {
    ...meal,
    calories:
      goal === "fat-loss" || goal === "beginner-reset"
        ? meal.calories
        : meal.calories,
  };
}
...
function buildDayMeals(goal, dayIndex, planIndex) {
  const breakfast = adjustMealForGoal(
    rotateItem(mealTemplates.breakfast, dayIndex + planIndex),
    goal
  );

  const lunch = adjustMealForGoal(
    rotateItem(mealTemplates.lunch, dayIndex + planIndex * 3 + 11),
    goal
  );

  const snack = adjustMealForGoal(
    rotateItem(mealTemplates.snack, dayIndex + planIndex * 5 + 23),
    goal
  );

  const dinner = adjustMealForGoal(
    rotateItem(mealTemplates.dinner, dayIndex + planIndex * 7 + 37),
    goal
  );
  ...
  if (goal === "muscle-gain") {
    const lateSnack = adjustMealForGoal(
      rotateItem(mealTemplates.snack, dayIndex + planIndex * 9 + 51),
      goal
    );
...
function buildSinglePlan(goal, planIndex) {
  ...
  return {
    ...
    days: WEEK_DAYS.map((day, dayIndex) => {
      const meals = buildDayMeals(goal, dayIndex, planIndex);
