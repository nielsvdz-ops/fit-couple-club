...
// Backwards compatibility for existing NutritionClient / RecipesClient files
// Some older components expect MEAL_GOALS as an array, not PLAN_GOALS as an object.
...
// Also support older goal names used in older pages/components.
export const GOAL_ALIASES = {
  lose_fat: "fat_loss",
  fatloss: "fat_loss",
  fat_loss: "fat_loss",
  build_muscle: "build_muscle",
  lean_muscle: "build_muscle",
...
export function normalizeGoal(goal) {
  ...
  if (GOAL_ALIASES[clean]) {
    return GOAL_ALIASES[clean];
  }
...
// Extra compatibility object for older code that expects grouped meal plans.
export const MEAL_PLANS_BY_GOAL = {
  fat_loss: FAT_LOSS_PLANS,
  lose_fat: FAT_LOSS_PLANS,
  build_muscle: BUILD_MUSCLE_PLANS,
  lean_muscle: BUILD_MUSCLE_PLANS,
  muscle_gain: BUILD_MUSCLE_PLANS,
  maintenance: MAINTENANCE_PLANS,
  performance: PERFORMANCE_PLANS,
};

// Older helper name support
export function getMealPlans(goal = "maintenance") {
  return getMealPlansByGoal(goal);
}
