...
// DUTCH / TRANSLATION ALIASES
// Makes EN + NL exercise names resolve to the same demo video.

const translatedExerciseAliases = {
  "bankdrukken": "bench-press",
  "incline bankdrukken": "incline-bench-press",
...
// UPDATED LOOKUP WITH TRANSLATION SUPPORT
export function resolveExerciseMedia(exerciseName = "") {
  const slug = slugify(exerciseName);

  const translatedSlug =
    translatedExerciseAliases[exerciseName?.toLowerCase?.()] ||
    translatedExerciseAliases[slug] ||
    slug;

  return (
    exerciseMediaMap[translatedSlug] ||
    exerciseMediaMap[slug] ||
    null
  );
}
